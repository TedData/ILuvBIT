"""
CSSE1001 Assignment 2
Semester 2, 2020
"""
from a2_support import *

# Fill these in with your details
__author__ = "{{user.name}} ({{user.id}})"
__email__ = ""
__date__ = ""

# Write your code here

class GameLogic:
    '''
    fdsajljfodsa


    '''
    def __init__(self, dungeon_name="game1.txt"):
        """Constructor of the GameLogic class.

        Parameters:
            dungeon_name (str): The name of the level.
        """
        self._dungeon = load_game(dungeon_name)
        self._dungeon_size = len(self._dungeon)

        #you need to implement the Player class first.
        self._player = Player(GAME_LEVELS[dungeon_name])

        #you need to implement the init_game_information() method for this.
        self._game_information = self.init_game_information()

        self._win = False

    def get_positions(self, entity):
        """ Returns a list of tuples containing all positions of a given Entity
             type.

        Parameters:
            entity (str): the id of an entity.

        Returns:
            )list<tuple<int, int>>): Returns a list of tuples representing the 
            positions of a given entity id.
        """
        positions = []
        for row, line in enumerate(self._dungeon):
            for col, char in enumerate(line):
                if char == entity:
                    positions.append((row,col))
            #要删除，list里是所有的点
        return positions

    # Write your code here
    def get_dungeon_size(self):#正确
        '''
        5长度
        '''
        return self._dungeon_size
    def init_game_information(self):#正确
        '''
        fdsaczx
        '''
        game_information={}
        try:
            self.get_player().set_position(self.get_positions(PLAYER)[0])
            for i, j in ((KEY,Key()),(DOOR,Door()),(WALL,Wall()),(MOVE_INCREASE,MoveIncrease())):#,
                for length in range(len(self.get_positions(i))):
                    game_information.update({self.get_positions(i)[length]:j})
        except :
            pass
        #self._player.set_position(self.get_positions(PLAYER)[0]) 
        return game_information
    def get_game_information(self):#正确
        '''
        fdsa
        '''
        return self._game_information
    def get_player(self):#正确
        '''
        fdsa
        '''
        return self._player
    def get_entity(self,tuple1):#correct
        '''
        fdsa
        '''
        return self.get_game_information().get(tuple1)
    def get_entity_in_direction(self, direction1):#correct      
        '''
        fdsa
        '''
        i,j=self.get_player().get_position()
        m,n=DIRECTIONS[direction1]
        new_position=(i+m,j+n)
        return self.get_game_information().get(new_position)
    def collision_check(self, direction1):#correct
        '''
        fdsa
        '''
        if str(self.get_entity_in_direction(direction1))==repr(Wall()):
            return True
        else:
            return False
    def new_position(self, direction1):#correct
        '''
        fdsa
        '''
        self.move_player(direction1)
        return self.get_player().get_position()
    def move_player(self, direction1):#correct
        '''
        fdsa
        '''
        m,n=self.get_player().get_position()
        i,j=DIRECTIONS[direction1]
        new_position=(m+i,n+j)
        self.get_player().set_position(new_position)           
    def check_game_over(self):#correct  
        '''
        if self.get_player().get_position() == self.get_positions(DOOR)[0]:           
            if self.get_player().get_inventory():
                return True 
        if self.get_player().moves_remaining() <= 1:
            return False
            True是游戏失败
            False并不代表游戏胜利
        '''
        if self.get_player().moves_remaining() <= 0 and not self.won():
            return True
        else:
            return False
    def set_win(self, bool1):#correct
        '''
        fdsa
        '''
        self._win=bool1
    def won(self):#correct
        '''
        if (not self.check_game_over()) and self.get_player().get_position() == self.get_positions(DOOR)[0]:           
            if self.get_player().get_inventory():
                self._win=True
        return self._win
        '''
        return self._win


class GameApp:
    '''
    fdasfdasfdsa
    '''
    def __init__(self):
        '''
        self._game=GameLogic()
        self._entity=Entity()
        self._key=Key()
        self._wall=Wall()
        self._item=Item()
        self._move_increase=MoveIncrease()
        self._door=Door()
        #self._display=Display(self._game_logic.get_game_information(),self._game_logic.get_dungeon_size())
        GameLogic().get_player().set_position(GameLogic().get_positions(PLAYER)[0])
        self.play()
        '''
        self.game_logic = GameLogic()
        self.game_logic.display_grid = Display(self.game_logic.get_game_information(), self.game_logic.get_dungeon_size())
        #self.game_logic.player_pos = self.game_logic.get_player().get_position()




    def play(self):
        '''
        while True:
            self.draw()
            s=input('Please input an action: ')
            if s==QUIT:
                b=input('Are you sure you want to quit? (y/n): ')
                if b=='y':
                    break
                elif b=='n':
                    continue
            elif s==HELP:
                print(HELP_MESSAGE)
            elif GameLogic().check_game_over():
                print(LOSE_TEST)
                break
            elif GameLogic().won():
                print(WIN_TEXT)
                break
            elif s[0]==INVESTIGATE:
                if s[-1] in (DIRECTIONS.keys()):
                    print('{0} is on the  {1} side.'.format(self.entity_in_direction(s[-1]),s[-1]))
                    GameLogic().get_player().change_move_count(-1)

            elif s in (DIRECTIONS.keys()):
                if not GameLogic().collision_check(s):
                    GameLogic().new_position(s)
                    self._door.on_hit(GameLogic())
                    if GameLogic().check_game_over() != None:
                        GameLogic().set_win(GameLogic().won(GameLogic().check_game_over()))
                        break                    
                else:
                    print(INVALID)
                GameLogic().get_player().change_move_count(-1)
                self._key.on_hit(GameLogic())
                self._move_increase.on_hit(GameLogic()) 
            
                #break
            elif s[0]==INVESTIGATE:
                if s[-1] in (DIRECTIONS.keys()):
                    print('{0} is on the  {1} side.'.format(self.entity_in_direction(s[-1]),s[-1]))
                    GameLogic().get_player().change_move_count(-1)
            
            elif s in (DIRECTIONS.keys()):
                if not GameLogic().collision_check(s):
                    GameLogic().new_position(s)
                    self._door.on_hit(GameLogic())
                    if GameLogic().check_game_over() != None:
                        GameLogic().set_win(GameLogic().won(GameLogic().check_game_over()))
                        break                    
                else:
                    print(INVALID)
                GameLogic().get_player().change_move_count(-1)
                self._key.on_hit(GameLogic())
                self._move_increase.on_hit(GameLogic())  
            
        '''
        while True:
            '''
            if self.game_logic.check_game_over():
                if self.game_logic.won():
                    print(WIN_TEXT)
                else:
                    print(LOSE_TEST)
                break
            '''
            if self.game_logic.check_game_over():
                print(LOSE_TEST)
                break
            elif self.game_logic.won():
                print(WIN_TEXT)
                break


            self.draw()
            s=input('Please input an action: ')
            if s==QUIT:
                b=input('Are you sure you want to quit? (y/n): ')
                if b=='y':
                    break
                elif b=='n':
                    continue
            elif s==HELP:
                print(HELP_MESSAGE)
            
            elif s[0]==INVESTIGATE and s[2:] in (DIRECTIONS.keys()):
                #if s[2:] in (DIRECTIONS.keys()):
                print('{0} is on the {1} side.'.format(self.game_logic.get_entity_in_direction(s[-1]),s[-1]))
                self.game_logic.get_player().change_move_count(-1)
                #else:
                	#print(INVALID)
            elif s in (DIRECTIONS.keys()):
                if self.game_logic.collision_check(s):
                    print(INVALID)
                else:
                    self.game_logic.new_position(s)
                    Key().on_hit(self.game_logic)
                    MoveIncrease().on_hit(self.game_logic)  
                    Door().on_hit(self.game_logic)

                          
                self.game_logic.get_player().change_move_count(-1)
            else:
                print(INVALID)
                







    def draw(self):#要优化
    	'''
		 def entity_in_direction(self, direction1):
        
        fdsa
        
        return GameLogic().get_entity_in_direction(direction1)   
		x=Display(self._game.get_game_information(),self._game.get_dungeon_size())
        x.display_game(self._game.get_player().get_position())
        x.display_moves(self._game.get_player().moves_remaining())
        

    	'''
    	self.game_logic.display_grid.display_game(self.game_logic.get_player().get_position())
    	self.game_logic.display_grid.display_moves(self.game_logic.get_player().moves_remaining())
        


class Entity(object):#正确不用改
    '''
    nihaovcxa
    '''

    def __init__(self):
        '''
        fdsa
        '''
        self._name = 'Entity'
        self._id = 'Entity'
        self._bool1 = True
    def get_id(self):
        '''
        fdsa
        '''
        return self._id
    def set_collide(self,bool1):
        '''
        fdsa
        '''
        self._bool1 = bool1
    def can_collide(self):
        '''
        fdsa
        '''
        return self._bool1	
    def __str__(self):
        '''
        fdsa
        '''
        x = "{0}('{1}')".format(self._name, self._id)
        return x
    def __repr__(self):
        '''
        fdsa
        '''
        x = "{0}('{1}')".format(self._name, self._id)
        return x

class Wall(Entity):#正确不用改
    '''
    fdajvna
    '''
    def __init__(self):
        '''
        fdsa
        '''
        self._name = 'Wall'
        self._id = '#'
        self._bool1 = False

class Item(Entity):#正确不用改
    '''
    fdvcxzee
    '''

    def __init__(self):
        '''
        fdsa
        '''
        super().__init__()
        self._name = 'Item'
        self._id = 'Entity'
        #self._bool1 = True
        #self._position1 = None
    def on_hit(self, game1):
        '''
        fdsa
        '''        
        raise NotImplementedError

class Key(Item):#正确不用改
    '''
    Key(Item)
    '''
    def __init__(self):
        '''
        key init
        '''
        super().__init__()
        self._name = 'Key'
        self._id = 'K'

    def on_hit(self, game1):
        '''
        on_hit
        '''
        try:
            if game1.get_player().get_position()==game1.get_positions(KEY)[0]:
                game1.get_player().add_item(self)
                game1._game_information.pop(game1.get_positions(KEY)[0])

        except:
            pass 
        '''
        if game1._player.get_position()==game1.get_positions(KEY)[0]:
            game1._player.add_item(str(Key()))
            game1._game_information.pop(game1.get_positions(KEY)[0])
            '''
            
class MoveIncrease(Item):#正确不用改
    '''
    fdason
    '''
    def __init__(self,moves=5):
        '''
        MoveIncrease
        '''
        super().__init__()
        self._name = 'MoveIncrease'
        self._id = 'M'
        self._moves = moves
    def on_hit(self, game1):
        '''
        on hit
        ''' 
        try:
            if game1.get_player().get_position()==game1.get_positions(MOVE_INCREASE)[0]:
                game1.get_player().change_move_count(5)
                game1._game_information.pop(game1.get_positions(MOVE_INCREASE)[0])

        except:
            pass#raise NotImplementedError 

class Door(Entity):#正确不用改
    '''
    viemizh
    '''

    def __init__(self):
        '''
        fdsa
        '''
        super().__init__()
        self._name = 'Door'
        self._id = 'D'
        self._bool1 = True
        self._position1 = None
    def on_hit(self, game1):
        '''
        if game1.get_player().get_position() == game1.get_positions(DOOR)[0]:           
            if not game1.get_player().get_inventory():
                print('You don\'t have the key!')
        '''
        if game1.get_player().get_position() == game1.get_positions(DOOR)[0]:           
            if not game1.get_player().get_inventory():
                print('You don\'t have the key!')
            else:
                game1.set_win(True)

class Player(Entity):#正确不用改
    '''
    cozemvcz
    '''

    def __init__(self, number1):
        '''
        fdsa
        '''
        super().__init__()
        self._name = 'Player'
        self._id = 'O'
        self._bool1 = True
        self._number1 = number1
        self._inventory1 = []
        self._position1 = None
    def set_position(self, position1):
        '''
        fdsa
        '''
        self._position1 = position1
    def get_position(self):
        '''
        fdsa
        '''
        return self._position1
    def change_move_count(self, change_num):
        '''
        fdsa
        '''
        self._change_num = change_num
        self._number1 += self._change_num
    def moves_remaining(self):
        '''
        fdsa
        '''
        return self._number1
    def get_inventory(self):
        '''
        fdsa
        '''
        return self._inventory1
    def add_item(self,item):
        '''
        fdsa
        '''
        self._inventory1.append(item)

def main():

    g=GameApp()
    g.play()



    
if __name__ == "__main__":
    main()