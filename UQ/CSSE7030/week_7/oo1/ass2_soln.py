...
class GameObject(object):
    ...
    def setPosition(self, x, y):
        """
        Sets the (x, y) position pair of the game object.

        GameObject.set_position(GameObject) -> (int, int)
        """
        self._x = x
        self._y = y
    ...
    def setName(self):
        ...
        GameObject.setName(GameObject) -> str
        """
        self._name = name
    ...
    def __str__(self):
        return "Char name: {0}  Pos: {1}".format(self.get_name(), self.get_position())
...
class Pokedex(GameObject):
      """
      Manages which Pokemon exist in which regions.
      """

    def __init__(self, region, Pokemon_names):
        """
        """
        super().__init__(x, y, name)
        pok_dict = {}
        for p_name in Pokemon_names:
            pok_dict[p_name]=False
    ...
    def register(self, pokemon_name):
        """
        """
        pok_dict[pokemon_name]=True

    def getPokemon(self):
        """
        """
        pok_list = []
        for k in sorted(pok_dict.keys()):
            pok_list.append((k,pok_dict[k]))
        
    def getUnregisteredPokemon(self):
        """
        """
        unreglist = []
        for k in sorted(pok_dict.keys()):
    ...
    def __str__(self):
        return "{0} Coastal \n  {1}  Registered: {2} \n  {3} Unregistered: {4}".format(self.getRegion(),
                                                                 len(reglist), reglist, len(unreglist), unreglist) 
...
class Model(object):
    ...
    def __init__(self):
        ...
        self.player = Player(50, 50, "player")
        self.monsters = [Monster(600, 50, "balloon", self),
                         TrackingMonster(50, 400, "purple", self),
                         RandomTrackingMonster(200, 300, "triangle", self)]
        self.items = [Weapon(400, 50, "sword", 10, 10, self),
    ...
    def get_monsters(self):
        ...
        self.monsters = [m for m in self.monsters if m.get_health() > 0]
    ...
    def player_take_item(self):
        ...
        cx, cy = self.player.get_position()
        for item in self.items:
            if not item.is_taken() and close_to(self.player.get_position(),
                                                item.get_position()):
                item.set_taken()
